...
export async function requestRefund(refundType, txnId, amount, refId) {
  try {
    ...
    const signature = md5(`${refundType}${RMS_CONFIG.merchantId}${refId}${txnId}${amount}${RMS_CONFIG.secretKey}`);
    ...
    console.log(`[Fiuu Refund] Requesting refund for Txn: ${txnId}, Amount: ${amount}, Ref: ${refId}`);
    ...
    const expectedSig = md5(`${data.RefundType}${data.MerchantID}${data.RefID}${data.RefundID}${data.TxnID}${data.Amount}${data.Status}${RMS_CONFIG.secretKey}`);
    const isSigValid = data.Signature && data.Signature.toLowerCase() === expectedSig.toLowerCase();
...
export async function queryRefundStatus(id, type = 'txn') {
  try {
    ...
    const signature = md5(`${id}${RMS_CONFIG.merchantId}${RMS_CONFIG.verifyKey}`);

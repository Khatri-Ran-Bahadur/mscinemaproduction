...
export default function TicketSelection() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        const configData = await shows.getConfigAndTicketPrice(
          cinemaId,
          showId,
        );
  ...
  const requiresConfirmation = (ticketTypeID) => {
    ...
    if (
      ticketTypeName.includes("CHILDREN") ||
      ticketTypeName.includes("CHILD") ||
      ticketTypeName.includes("KIDS")
    ) {
      return true;
    }
    if (
      ticketTypeName.includes("SENIOR") ||
      ticketTypeName.includes("SENIOR-CITIZEN")
    ) {
      return true;
    }
  ...
  const handleGoBack = () => {
    ...
    const showTimeParam = showTimeDetails?.showTime
      ? encodeURIComponent(showTimeDetails.showTime)
      : "";
    const showDateParam = showTimeDetails?.showDate
      ? encodeURIComponent(showTimeDetails.showDate)
      : "";
    const expType = experienceType || movieDetails?.type || "2D";

    if (showTimeParam && showDateParam && showId) {
      localStorage.setItem(
        "lastSelectedShowTime",
        JSON.stringify({
  ...
  const getShowDescription = (categoryName) => {
    ...
    if (
      categoryName.includes("Senior") ||
      categoryName.includes("Senior-Citizen")
    ) {
  ...
  const proceedToSeatSelection = () => {
    ...
    router.push(
      `/seat-selection?cinemaId=${encrypted.cinemaId}&showId=${encrypted.showId}&movieId=${encrypted.movieId}`,
    );
  };
            ...
            <div className="bg-[#232323] rounded-b-lg">
              {ticketData.priceDetails && ticketData.priceDetails.length > 0 ? (
                ticketData.priceDetails.map((price, idx) => {
                  ...
                  const description = getTicketTypeDescription(
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-center text-sm text-white/90">
                        RM{" "}
                        {(
                          price.totalTicketPrice * (isTwinSeats ? 2 : 1)
                        ).toFixed(2)}
                      </div>
                      <div className="flex items-center justify-center gap-3">
                        ...
                        <span className="text-sm w-6 text-center text-white font-medium">
                          {ticketCount}
                        </span>
                        <button
              ...
              <div className="p-6">
                ...
                <div className="text-center mb-6">
                  <p className="text-[#FAFAFA] text-sm leading-relaxed">
                    Please confirm you are under this particular category{" "}
                    <span className="font-bold text-[#FFCA20]">
                    ...
                    , verification will be done at the checkpoint.
                  </p>
                </div>
